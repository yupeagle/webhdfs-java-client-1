...
public class KerberosWebHDFSConnection implements WebHDFSConnection {
	...
	protected static final Logger logger = LoggerFactory
			.getLogger(KerberosWebHDFSConnection.class);
	...
	public static synchronized Token generateToken(String srvUrl, String princ,
		...
		try {
			...
			System.setProperty("java.security.krb5.realm", "EDF.FR");
	        System.setProperty("java.security.krb5.kdc","noeyy0ln.adam.adroot.edf.fr");

			HttpURLConnection conn = new AuthenticatedURL(
					new KerberosAuthenticator2(princ, passwd, "HTTP/big3.big"))
					.openConnection(new URL(new URL(srvUrl),
							"/webhdfs/v1/?op=GETHOMEDIRECTORY"), newToken);
			conn.connect();
